# VS Code Complete Clean Reinstallation Script - FIXED VERSION
# Addresses extension conflicts, corrupted settings, and workspace contamination

Write-Host "=== VS Code Complete Clean Reinstallation ===" -ForegroundColor Cyan
...
if (Test-Path "c:\net\.vscode\") {
    ...
    Copy-Item "c:\net\.vscode\" "$backupFolder\.vscode\" -Recurse -Force -ErrorAction SilentlyContinue
}

# Backup current workspace file
if (Test-Path "c:\net\NET-est.code-workspace") {
    Write-Host "  Backing up workspace file..." -ForegroundColor White
    Copy-Item "c:\net\NET-est.code-workspace" "$backupFolder\NET-est.code-workspace" -Force -ErrorAction SilentlyContinue
}
...
Write-Host "STEP 4: Analyzing Python environment..." -ForegroundColor Green
...
foreach ($pattern in $pythonPaths) {
    ...
    if ($found) {
        ...
        Write-Host "    No installations found for pattern: $pattern" -ForegroundColor Gray
    }
}
...
Write-Host "2. Open the optimized workspace: code NET-est-optimized.code-workspace" -ForegroundColor White
...
Write-Host "   - GitHub Copilot (for development assistance)" -ForegroundColor Gray
...
Write-Host "- Use the activate-dev-env.ps1 script before development" -ForegroundColor White

Write-Host ""
Write-Host "VS Code reinstallation completed successfully!" -ForegroundColor Green
Write-Host ""
Write-Host "To restore conversation context: .\restore-context.ps1" -ForegroundColor Cyan
