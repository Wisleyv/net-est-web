...
class MicroStageEvaluator:
    ...
    def _evaluate_lexical_simplification(self, features: StrategyFeatures, source_text: str, target_text: str, threshold: float = 0.50) -> Optional[StrategyEvidence]:
        ...
        # Look for word length reduction
        import numpy as np
        ...
        if confidence >= threshold:
            ...
            # This ensures SL+ strategies are spread across the text
            positions = []
            src_sentences = self._split_into_sentences(source_text)

            # Distribute SL+ positions across different sentences
            if src_sentences:
                # Place SL+ at different positions based on text length
                num_positions = min(2, len(src_sentences))  # Up to 2 positions for SL+

                for i in range(num_positions):
                    # Distribute across the text (not just at beginning)
                    sentence_idx = min(i * 2, len(src_sentences) - 1)  # Every other sentence
