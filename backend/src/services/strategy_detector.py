...
class EvidenceBasedClassifier:
    ...
    def __init__(self):
        # Strategy detection thresholds and rules - PRIORITIZED ORDER for canonical detection
        # Phase A: Core semantic strategies (highest priority)
        # Phase B: Structural strategies
        # Phase C: Enhanced analysis strategies
        self.strategy_rules = {
            # PHASE A: Core semantic strategies (detect first)
            'SL+': self._classify_lexical_simplification,      # Adequação de Vocabulário - our new semantic algorithm
            'RP+': self._classify_sentence_fragmentation,      # Fragmentação Sintática
            'RF+': self._classify_global_rewriting,           # Reescrita Global
            'MT+': self._classify_title_optimization,         # Otimização de Títulos
            'RD+': self._classify_content_structuring,        # Estruturação de Conteúdo
            
            # PHASE B: Enhanced semantic strategies (detect second)
            'MOD+': self._classify_perspective_reinterpretation, # Reinterpretação Perspectiva
            'DL+': self._classify_positional_reorganization,    # Reorganização Posicional
            'EXP+': self._classify_explicitness,               # Explicitação e Detalhamento
            'IN+': self._classify_insertion_handling,          # Manejo de Inserções
            
            # PHASE C: Lower priority strategies (detect last)
            'TA+': self._classify_referential_clarity,         # Clareza Referencial
            'MV+': self._classify_voice_change,                # Alteração da Voz Verbal
            'AS+': self._classify_meaning_change,              # Alteração de Sentido (tolerance strategy)
            
            # PHASE D: Human annotation strategies (require manual validation)
            'OM+': self._classify_selective_omission,          # Omissão Seletiva (human validation required)
            'PRO+': self._classify_problem_processing          # Processamento de Problemas (human annotation)
        }
