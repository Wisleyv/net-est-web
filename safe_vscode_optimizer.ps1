...
if ($vscodeProcesses.Count -gt 12 -or $totalMemoryMB -gt 2000) {
    $severityLevel = if ($totalMemoryMB -gt 3000 -or $vscodeProcesses.Count -gt 15) { "CRITICAL" } else { "WARNING" }
    $severityColor = if ($totalMemoryMB -gt 3000 -or $vscodeProcesses.Count -gt 15) { "Red" } else { "Yellow" }
    $severityIcon = if ($totalMemoryMB -gt 3000 -or $vscodeProcesses.Count -gt 15) { "🔴" } else { "🟡" }
    
    Write-Host "$severityIcon $severityLevel`: VS Code performance issues detected" -ForegroundColor $severityColor
    Write-Host "  Processes: $($vscodeProcesses.Count) | Memory: $([math]::Round($totalMemoryMB, 0)) MB" -ForegroundColor $severityColor
    
    # Check for extreme memory usage
    $extremeProcesses = $vscodeProcesses | Where-Object { $_.MemoryMB -gt 800 }
    if ($extremeProcesses.Count -gt 0) {
        Write-Host "🚨 EXTREME MEMORY USAGE DETECTED:" -ForegroundColor Red
        $extremeProcesses | ForEach-Object {
            Write-Host "  PID $($_.Id): $($_.MemoryMB) MB - This is causing your lag!" -ForegroundColor Red
        }
    }
    Write-Host ""
    ...
    Write-Host "1. Close VS Code normally (safest)" -ForegroundColor White
    Write-Host "2. Kill high-memory processes (>400MB) - RECOMMENDED for your situation" -ForegroundColor White
    ...
    switch ($choice) {
        ...
        "2" {
            $highMemoryProcesses = $vscodeProcesses | Where-Object { $_.MemoryMB -gt 400 }
            if ($highMemoryProcesses.Count -gt 0) {
                Write-Host "High memory processes to be terminated (>400MB):" -ForegroundColor Yellow
                $highMemoryProcesses | ForEach-Object {
                    Write-Host "  PID $($_.Id): $($_.ProcessName) - $($_.MemoryMB) MB - CPU: $($_.CPUTime)s" -ForegroundColor Red
                    if ($_.MainWindowTitle) {
                        Write-Host "    Window: $($_.MainWindowTitle)" -ForegroundColor Gray
                    }
                }
                Write-Host ""
                Write-Host "⚠️  NOTE: This will target the 1GB+ process that's causing your lag!" -ForegroundColor Yellow
                $confirm = Read-Host "Confirm termination? (y/N)"
                if ($confirm -eq 'y' -or $confirm -eq 'Y') {
                    $highMemoryProcesses | ForEach-Object {
                        try {
                            Stop-Process -Id $_.Id -Force
                            Write-Host "  ✅ Terminated PID $($_.Id) - Freed $($_.MemoryMB) MB" -ForegroundColor Green
                        }
                        catch {
                    ...
                    Write-Host ""
                    Write-Host "🎉 Memory optimization completed! VS Code should be much faster now." -ForegroundColor Green
                    Write-Host "💡 If VS Code seems unstable, just reopen it - your work is auto-saved." -ForegroundColor Cyan
                }
            } else {
